...
CREATE PROCEDURE spGetVisitItemsByIDorPlate(  @aintVisitId udtIdentifier, @astrVisitPlate varchar(32)   )
 ...
 AS
	...
	*/
	...
	DECLARE @lint_visitmAXP  udtIdentifier
     ...
     --- buscar por placas 
      IF LEN(@astrVisitPlate)> 3 
	      BEGIN
               ...
               --doble busqueda 
               -- primera
                   SELECT @lint_visitmAXP = ISNULL(MAX(tblclsVisit.intVisitId),0)
                   FROM tblclsVisit
               ...
               -- segunda 
                IF @lint_visitmAXP > 0 
                 BEGIN                 
		               ...
		               WHERE tblclsVisit.intVisitId  = @lint_visitmAXP
		               
		               
		        END -- IF @lint_visitmax > 0 
		               
	      END  --      IF LEN(@astrVisitPlate)> 3 
      ELSE
